...
#pragma warning restore 0219
        ...
        private Expression ParseMultiplicative()
        {
            ...
            while (this.token.id == TokenId.Asterisk || this.token.id == TokenId.Slash ||
                   this.token.id == TokenId.Percent || TokenIdentifierIs("mod"))
            {
                Token op = this.token;
                NextToken();
        ...
        private Expression ParseIdentifier()
        {
            ...
            if (this.it != null) return ParseMemberAccess(null, this.it);
            throw ParseError(Res.UnknownIdentifier, this.token.text);
        }
        ...
        private Expression[] ParseArgumentList()
        {
            ValidateToken(TokenId.OpenParen, Res.OpenParenExpected);
            NextToken();
            Expression[] args = this.token.id != TokenId.CloseParen ? ParseArguments() : new Expression[0];
            ValidateToken(TokenId.CloseParen, Res.CloseParenOrCommaExpected);
            NextToken();
            return args;
        }
        ...
        {
            while (Char.IsWhiteSpace(this.ch)) NextChar();
            TokenId t;
            ...
            switch (this.ch)
            {
                ...
                case '\'':
                    ...
                    {
                        NextChar();
                        while (this.textPos < this.textLen && this.ch != quote) NextChar();
                        if (this.textPos == this.textLen)
                            throw ParseError(this.textPos, Res.UnterminatedStringLiteral);
                        NextChar();
                    } while (this.ch == quote);
                ...
                default:
                    ...
                    if (Char.IsDigit(this.ch))
                    {
                        ...
                        if (this.ch == 'F' || this.ch == 'f') NextChar();
                        break;
                    }
                    if (this.textPos == this.textLen)
                    {
        ...
        {
            return this.token.id == TokenId.Identifier &&
                   String.Equals(id, this.token.text, StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            if (!Char.IsDigit(this.ch)) throw ParseError(this.textPos, Res.DigitExpected);
        }

        private void ValidateToken(TokenId t, string errorMessage)
        {
            if (this.token.id != t) throw ParseError(errorMessage);
        }

        private void ValidateToken(TokenId t)
        {
            if (this.token.id != t) throw ParseError(Res.SyntaxError);
        }
